...
Validates the Phase 1 `.tflite` model off real audio before any ESP32 work:
...
```
Expects Phase 1's output at `../vikramedge_phase1/artifacts/vikramedge_phase1_int8.tflite`
...
```bash
python eval_wav.py \
    --tflite_path ../vikramedge_phase1/artifacts/vikramedge_phase1_int8.tflite \
    --data_dir ../vikramedge_phase1/data/processed/testing
```

Reports: accuracy, **False Acceptance Rate** (silence/unknown misfired as
yes/no), **False Rejection Rate** (yes/no missed), per-inference latency
...
the same rolling-buffer pattern the ESP32 will use in Phase 5, rather than
a push-to-talk recording. Say "yes" or "no" to see detections print live,
...
- Say "yes"/"no" repeatedly and check the **miss rate** and reported latency.
...
these PC numbers hold up outside the Speech Commands distribution.
